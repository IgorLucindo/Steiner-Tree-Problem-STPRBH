...
Maxflow::Maxflow(int n, int m, list<pair<u_int, u_int> >& arcs) :
		nsource(0), nsink(0), q_read(0), q_write(0), qp_first(0), qp_last(0), lmax(0), n_up(0), n_rel(0), n_push(0)
{
	this->n = n;
	this->m = m;
	...
	for (int i = 0; i < n - 1; i++)
	{ // scanning all nodes except the last
	...
	if (bos != NULL)
	{
		...
		while (1)
		{
			...
			if (a != NULL)
			{
				...
				{
					if (!a)
						cout << "a is NULL, Excess left is " << i->excess << endl;
